...
func TestBaseAppBlockListFiltersTypeAndPreservesPagination(t *testing.T) {
	factory, stdout, reg := newExecuteFactory(t)
	registerListStub := func() {
		reg.Register(&httpmock.Stub{
			Method: "GET",
			URL:    "/open-apis/base/v3/base_apps/app_x/pages/pge_x/blocks?page_size=20",
			Body: map[string]interface{}{
				"code": 0,
				"data": map[string]interface{}{
					"has_more":   true,
					...
					"total":      4,
					"items": []interface{}{
						map[string]interface{}{"widget_id": "wgt_1", "name": "Sales", "type": "statistics"},
						map[string]interface{}{"widget_id": "wgt_2", "name": "Trend", "type": "line"},
						map[string]interface{}{"widget_id": "wgt_3", "name": "Orders", "type": "STATISTICS"},
						map[string]interface{}{"widget_id": "wgt_4", "name": "Notes", "type": "text"},
					},
				},
			},
		})
	...
	err := runShortcut(t, BaseAppBlockList, []string{
		"+app-block-list",
		"--app-token", "app_x",
		"--page-id", "pge_x",
		"--type", "statistics",
	...
	if !ok || len(items) != 2 {
		t.Fatalf("items=%#v, want two statistics blocks", data["items"])
	}
	for _, item := range items {
		block, ok := item.(map[string]interface{})
		if !ok || !strings.EqualFold(common.GetString(block, "type"), "statistics") {
			t.Fatalf("unexpected filtered item: %#v", item)
		}
	}
	if data["has_more"] != true || data["page_token"] != "next_x" {
		t.Fatalf("pagination fields changed: %#v", data)
	}
	...
	items, ok = data["items"].([]interface{})
	if !ok || len(items) != 1 {
		t.Fatalf("text items=%#v, want one text block", data["items"])
	}
	block, ok := items[0].(map[string]interface{})
	if !ok || common.GetString(block, "type") != "text" {
		t.Fatalf("text filtered item=%#v, want type text", items[0])
	}
}
