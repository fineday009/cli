...
- Blocked case (structured):
  - blocked_case: im.search.read_after_write_index_delay
  - affected_commands: `im +chat-search`, `im +messages-search`
  - not_fixing_with_hint: true
  - coverage_rule: do not assert immediate search visibility after create/send
  - next_fixture_requirement: stable historical chat/message fixture, or an explicit index-wait strategy
