...
- BaseApp（应用模式）把 Base 数据组织成页面和组件。应用命令使用 `+app-create/get/rename`，页面使用 `+app-page-*`。页面/组件命令吃 `app_token` + `page_id`，表/字段/记录命令吃 `base_token`；唯一例外是 `+app-block-get-data`。`+app-create` 会创建空 Base 并通过 `+workspace-move-in` 移入同一 Workspace；部分失败时不得重复创建 App，应按返回的 `retry.command` 续跑。列表使用 `type=list + sub_type`，每个列表至多一个同 Workspace Base。详见 [lark-base-baseapp.md](references/lark-base-baseapp.md) 与 [lark-base-baseapp-block-data-config.md](references/lark-base-baseapp-block-data-config.md)。
